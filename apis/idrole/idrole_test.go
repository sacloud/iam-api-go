...
func setup(t *testing.T, v any, s ...int) (*require.Assertions, IDRoleAPI) {
	assert := require.New(t)
	client := iam_test.NewTestClient(v, s...)
	api := NewIdRoleOp(client)
	return assert, api
}

func TestNewIdRoleOp(t *testing.T) {
	assert, api := setup(t, make(map[string]any), http.StatusAccepted)
	assert.NotNil(api)
}

func TestList(t *testing.T) {
	var expected v1.IDRolesGetOK
	expected.SetFake()
	expected.SetItems(make([]v1.IdRole, 1))
